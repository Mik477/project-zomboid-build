...
$sourceModRoot = Join-Path $repositoryRoot 'src\mods\PZPerformanceDiagnostics'
...
if ([string]$localConfiguration.projectZomboid.exactGameVersion -ne '42.20.3') {
    throw "Diagnostics require Project Zomboid 42.20.3; local configuration reports $($localConfiguration.projectZomboid.exactGameVersion)."
}
if ([string]$localConfiguration.steam.buildId -ne '24775755') {
    throw "Diagnostics require Steam build 24775755; local configuration reports $($localConfiguration.steam.buildId)."
}
if ([string]$manifest.compatibility.exactGameVersion -ne '42.20.3' -or
    [string]$manifest.compatibility.steamBuildId -ne '24775755') {
    throw 'Repository compatibility metadata no longer matches the diagnostics target.'
}
...
$buildRoot = Join-Path $env:LOCALAPPDATA 'project-zomboid-build\build\PZPerformanceDiagnostics'
...
if (-not $DestinationModRoot) {
    $DestinationModRoot = Join-Path $env:LOCALAPPDATA 'project-zomboid-build\prototype-output\PZPerformanceDiagnostics'
}
...
$jarPath = Join-Path $jarDirectory 'PZPerformanceDiagnostics.jar'
